...
function PaynmentModal({modalData , openModal , paynmentHandler}) {
    ...
    const [modalDatas ,setModalDatas] = useState(JSON.parse(localStorage.getItem('MODAL_DATA')) || modalData)
  return (
    <div className={`h-fit w-[75%] flex flex-col px-[25px] pb-[20px] items-center transition-all duration-1000  bg-white rounded-xl border shadow-lg absolute left-1/2 right-1/2 -translate-x-1/2 ${openModal ? '-translate-y-[1000px]' : 'translate-y-10 fixed'}`}>
        ...
        <p>Langsung bayar pesanan punya {modalDatas.name}</p>
        <p className='absolute right-10 top-10 cursor-pointer' onClick={paynmentHandler}>X</p>
