...
function History({ orderselesai, orderitems , tax }) {
    ...
    for (
        let i = 1;
        i <= Math.ceil(dataOrderSelesai.length / postPerPage);
        i++
    ) {
        pageNumbers.push(i);
    }
    ...
    const currentPosts = dataOrderSelesai.slice(
        indexOfFirstPost,
        indexOfLastPost
    );
    ...
    return (
        <BodyLayout className={"pt-[40px] px-[40px]"}>
                        ...
                        currentPosts.map((orderselesai) => {
                            return (
                                <tr className="h-fit border-bottom-1">
                                    <TableData
                                        text={orderselesai.customer_name}
                                    />
            ...
            {/* modal */}
            <div
                className={`w-[900px] h-[500px] px-[25px] py-[25px] flex flex-col items-center absolute left-1/2 right-1/2 -translate-x-1/2 transition-all duration-1000 rounded-xl bg-white border shadow-lg ${
                    openModal ? "-translate-y-[1000px]" : "-translate-y-0"
                }`}
            >
                ...
                <div className="w-full h-full mt-[10px] flex gap-x-[30px]">
                    ...
                    <div>
                        <div className="w-[330px] h-[60%] border rounded-xl bg-white flex flex-col  py-[20px] px-[25px] mb-[10%]">
                            {itemPost.length == 0 ? (
                                <p>TIdak ada data</p>
                            ) : (
